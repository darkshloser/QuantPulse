...
const MARKET_DATA_API_BASE =
    (import.meta.env.VITE_MARKET_DATA_API_URL as string) ||
    "http://localhost:8002";
const ANALYZER_API_BASE =
    (import.meta.env.VITE_ANALYZER_API_URL as string) ||
    "http://localhost:8003";
const NOTIFIER_API_BASE =
    (import.meta.env.VITE_NOTIFIER_API_URL as string) ||
    "http://localhost:8004";
...
const setupInterceptors = () => {
    [authClient, symbolClient, marketDataClient, analyzerClient, notifierClient].forEach(
        (client) => {
...
export const symbolAPI = {
    async listSymbols(search?: string, limit = 100, offset = 0): Promise<{ symbols: Symbol[]; total: number }> {
        const params: Record<string, string | number> = { limit, offset };
        if (search) params.search = search;
        const response = await symbolClient.get("/symbols", { params });
        return response.data;
    },
    ...
    async importSecSymbols(): Promise<{
        exchange: string;
        processed: number;
        inserted: number;
        updated: number;
        ...
        const response = await symbolClient.post("/symbols/import/sec", {
            source: "SEC_OFFICIAL",
...
export const marketDataAPI = {
    async getMarketData(symbol: string) {
        const response = await marketDataClient.get(`/market-data/${symbol}`);
        return response.data;
    },

    async fetchSymbolData(symbol: string) {
        const response = await marketDataClient.post(`/fetch/${symbol}`);
        return response.data;
    },

    async fetchAllData() {
        const response = await marketDataClient.post("/fetch-all");
        return response.data;
    },
};
